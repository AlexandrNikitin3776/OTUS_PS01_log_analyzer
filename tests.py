...
class Testlogparsing(unittest.TestCase):
    ...
    def test_log_good(self):
        logline = "1.196.116.32 -  - [29/Jun/2017:03:50:22 +0300] \"GET " \
                  ...
                  "\"dc7161be3\" 0.390"
        parseresult = {"request_url": "/api/v2/banner/25019354",
                       "request_time": 0.390}
        testresult = log_analyzer.parselog(logline, log_analyzer.log_format,
                                           log_analyzer.request_format)
        self.assertEqual(testresult["request_url"], parseresult["request_url"])
        self.assertTrue(abs(testresult["request_time"]
                            - parseresult["request_time"]) < 0.002)
...
class Testanalyzing(unittest.TestCase):
    ...
    def test_empty_file(self):
        ...
        testlog.close()
        result = log_analyzer.analyzelog(emptytestfilename,
    ...
    def test_tenline_file(self):
        ...
        logline = "1.196.116.32 -  - [29/Jun/2017:03:50:22 +0300] \"GET " \
                  ...
                  "10.5\" \"-\" \"1498697422-2190034393-4708-9752759\" " \
                  "\"dc7161be3\" 0.390"
        analyzeresult = {'/api/v2/banner/25019354':
                         {'count': 10, 'count_perc': 100.0,
                          'time_avg': 0.39, 'time_max': 0.39,
                          'time_med': 0.39, 'time_perc': 100.0,
                          'time_sum': 3.9, 'url': '/api/v2/banner/25019354'}}

        testlog = open(tenlinefilename, mode="wt")
        testlog.writelines([logline, "\n"] * 9)
        testlog.write(logline)
        testlog.close()
        result = log_analyzer.analyzelog(tenlinefilename,
                                         log_analyzer.log_format,
                                         log_analyzer.request_format,
                                         log_analyzer.config)
    ...
    def test_errorlogfile(self):
        ...
        testlog.write("\n")
        testlog.close()
        result = log_analyzer.analyzelog(errorlogfilename,
                                         log_analyzer.log_format,
                                         log_analyzer.request_format,
                                         log_analyzer.config)
